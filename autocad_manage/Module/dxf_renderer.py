...
class DXFRenderer(DXFLoader):
    ...
    def updateImageTrans(self, fix_image_size=True):
        ...
        if diff_point.x > 0:
            self.scale = min(
                self.scale, 1.0 * (self.image_width - 2.0 * self.free_width) /
                diff_point.x)
            render_scale = min(render_scale,
                               1.0 * self.image_width / diff_point.x)
        if diff_point.y > 0:
            self.scale = min(
                self.scale, 1.0 * (self.image_height - 2.0 * self.free_width) /
                diff_point.y)
            render_scale = min(render_scale,
                               1.0 * self.image_height / diff_point.y)
    ...
    def getImagePosition(self, world_potision):
        if not self.trans_point.isFinite():
            return Point(int(self.free_width), int(self.free_width),
                         int(self.free_width))

        image_position = Point(
            int(self.free_width +
                (world_potision.x + self.trans_point.x) * self.scale),
            int(self.free_width +
                (world_potision.y + self.trans_point.y) * self.scale),
            int(self.free_width +
                (world_potision.z + self.trans_point.z) * self.scale))
    ...
    def getWorldPosition(self, image_position):
        ...
        world_potision = Point(
            1.0 * (image_position.x - self.free_width) / self.scale -
            self.trans_point.x,
            1.0 * (image_position.y - self.free_width) / self.scale -
            self.trans_point.y,
            1.0 * (image_position.z - self.free_width) / self.scale -
            self.trans_point.z)
        return world_potision

    def drawPoint(self, point, color):
        point_in_image = self.getImagePosition(point)
        cv2.circle(self.image, (point_in_image.x, point_in_image.y), 1,
                   tuple(color), 4)
        return True
    ...
    def drawLine(self, line, color):
        ...
        cv2.line(self.image, (start_point_in_image.x, start_point_in_image.y),
                 (end_point_in_image.x, end_point_in_image.y), tuple(color), 1,
                 4)
        return True
    ...
    def drawCircle(self, circle, color):
        ...
        cv2.circle(self.image, (center_in_image.x, center_in_image.y),
                   radius_in_image, tuple(color), 1, 8, 0)
        return True
    ...
    def renderFrameWithAllShape(self, fix_image_size=True):
        ...
        self.image = np.zeros(
            (self.render_image_height, self.render_image_width, 3),
            dtype=np.uint8)
